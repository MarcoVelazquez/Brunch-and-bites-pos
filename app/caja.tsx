...
import { getAllProducts, simpleAddSale, simpleAddSaleItem } from './lib/database.refactor';
...
export default function CajaScreen() {
    ...
    const handleCobrar = async () => {
        ...
        try {
            ...
            const userId = user?.id || 1; // Usar ID del usuario actual o 1 por defecto
            const businessName = "Brunch & Bites";
            
            const saleId = await simpleAddSale(userId, total, pago, calculatedChange, businessName);
            console.log('✅ Venta guardada con ID:', saleId);
            
            // Guardar los items de la venta
            for (const item of items) {
                await simpleAddSaleItem(saleId, item.id, item.name, item.quantity, item.price);
                console.log('📦 Item guardado:', item.name, 'x', item.quantity);
            }
            ...
            Alert.alert(
                'Venta procesada', 
                `Total: $${total.toFixed(2)}\nRecibido: $${pago.toFixed(2)}\nCambio: $${calculatedChange.toFixed(2)}\n\n⚠️ Error al guardar en recibos, pero el pago fue procesado`,
                [
                    {
                        text: 'Nueva venta',
                        onPress: () => {
